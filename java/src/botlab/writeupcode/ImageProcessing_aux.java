...
public class ImageProcessing_aux extends VisEventAdapter
{
	...
	public ImageProcessing_aux(ImageSource _is, boolean _dispGUI)
	{
		is = _is;
...
Line 0, start:(1026.372020,895.778878) end:(791.627980,558.295196)
	...
	{
		ArrayList<ArrayList<double []> > bl = alf.getLineSegs();
 
		for (int i=0; i< bl.size(); i++)
		{
			System.out.printf("Line %d, start:(%f,%f) end:(%f,%f)\n",i,
				bl.get(i).get(0)[0],bl.get(i).get(0)[1],bl.get(i).get(1)[0],bl.get(i).get(1)[1]);
	...
	public static void main(String args[]) throws IOException
	{
		...
		new ImageProcessing_aux(is, true).run();
	}
